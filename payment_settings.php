...
Commission Percentage Amount you get from vendor sales.
...
Minimum balance required to make a withdraw request. Value 0 to set no minimum limits.
...
Days, ( Make vendor order matured to withdraw his commision)<br>

NUMBER OF DAYS BEFORE Vendor Commision FROM Orders CAN BE AVAILABLE FOR WITHDRAWAL BY Him
...
<option value="1" <?php if($enable_paypal == 1){ echo "selected"; } ?>> Yes </option>
...
Enter Paypal Bussiness Email For Receiving Paypal Payments And Sending Earnings To Vendors Paypal Accounts.
...
<input type="radio" name="paypal_sandbox" value="1" <?php if($paypal_sandbox == 1){ echo "checked"; } ?> required> On 

</label>

<label class="control-label">

<input type="radio" name="paypal_sandbox" value="0" <?php if($paypal_sandbox == 0){ echo "checked"; } ?> required> Off 
...
<label class="col-md-3 control-label"> Paypal Payout : </label>

<div class="col-md-7">

<div class="form-group"><!-- form-group Starts -->

<label> Paypal App Client ID: </label>
...
<label> Paypal App Client Secret: </label>
...
<option value="yes" <?php if($enable_stripe == "yes"){ echo "selected"; } ?>> Yes </option>
...
<input type="submit" name="update_payment_settings" class="btn btn-primary form-control" value="Update Payment Settings">
...
alert('Your Stripe Settings Have Been Updated Successfully.');
